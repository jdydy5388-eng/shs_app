...
class RadiologyHandler {
    ...
    try {
      ...
      final data = rows
          .map(
            (r) => {
              'id': r[0]?.toString(),
              'doctorId': r[1]?.toString(),
              'patientId': r[2]?.toString(),
              'patientName': r[3],
              'modality': r[4],
              'bodyPart': r[5],
              'status': r[6],
    ...
    try {
      ...
      await conn.execute(
        ...
        substitutionValues: {
          ...
          'notes': body['notes'],
          'requestedAt':
              body['requestedAt'] ?? DateTime.now().millisecondsSinceEpoch,
          'scheduledAt': body['scheduledAt'],
          'completedAt': body['completedAt'],
        },
      );
      return ResponseHelper.success(
        data: {'message': 'Radiology request created'},
      );
    } catch (e) {
  ...
  Future<Response> _updateRequest(Request request, String id) async {
    try {
      ...
      if (!user.isAdmin) {
        final owns = await conn.query(
          'SELECT 1 FROM radiology_requests WHERE id=@id AND doctor_id=@doc LIMIT 1',
      ...
      if (body.containsKey('scheduledAt'))
        setField('scheduled_at', body['scheduledAt']);
      if (body.containsKey('completedAt'))
        setField('completed_at', body['completedAt']);
      if (fields.isEmpty)
        return ResponseHelper.success(data: {'message': 'Nothing to update'});
      await conn.execute(
        'UPDATE radiology_requests SET ${fields.join(', ')} WHERE id = @id',
        substitutionValues: values,
      );
      return ResponseHelper.success(
        data: {'message': 'Radiology request updated'},
      );
    } catch (e) {
  ...
  Future<Response> _updateRequestStatus(Request request, String id) async {
    try {
      ...
      return ResponseHelper.success(
        data: {'message': 'Radiology status updated'},
      );
    } catch (e) {
  ...
  Future<Response> _getReports(Request request) async {
    try {
      ...
      if (!user.isAdmin && requestId == null) {
        if (user.isDoctor) {
          query +=
              ' AND request_id IN (SELECT id FROM radiology_requests WHERE doctor_id=@doc)';
          values['doc'] = user.id;
        } else if (user.isPatient) {
          query +=
              ' AND request_id IN (SELECT id FROM radiology_requests WHERE patient_id=@pat)';
          values['pat'] = user.id;
        } else {
          return Response.forbidden('Unauthorized');
        }
      ...
      final data = rows
          .map(
            (r) => {
              'id': r[0]?.toString(),
              'requestId': r[1]?.toString(),
              'findings': r[2],
              'impression': r[3],
              'attachments': _parseAttachments(r[4]),
              'createdAt': r[5],
            },
          )
          .toList();
  ...
  Future<Response> _createReport(Request request) async {
    try {
      ...
      if (!user.isAdmin) {
        final owns = await conn.query(
          'SELECT 1 FROM radiology_requests WHERE id=@rid AND doctor_id=@doc LIMIT 1',
      ...
      await conn.execute(
        ...
        substitutionValues: {
          ...
          'attachments': body['attachments'] != null
              ? jsonEncode(body['attachments'])
              : null,
          'createdAt':
              body['createdAt'] ?? DateTime.now().millisecondsSinceEpoch,
        },
      );
      return ResponseHelper.success(
        data: {'message': 'Radiology report created'},
      );
    } catch (e) {
