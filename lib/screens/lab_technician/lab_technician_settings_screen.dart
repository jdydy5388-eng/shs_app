...
class LabTechnicianSettingsScreen extends StatefulWidget {
  const LabTechnicianSettingsScreen({super.key});

  @override
  State<LabTechnicianSettingsScreen> createState() =>
      _LabTechnicianSettingsScreenState();
}

class _LabTechnicianSettingsScreenState
    extends State<LabTechnicianSettingsScreen>
  ...
  Future<void> _loadStats() async {
    try {
      ...
      final completedRequests = requests
          .where((r) => (r as LabRequestModel).status == LabRequestStatus.completed)
          .length;
      final pendingRequests = requests
          .where((r) => (r as LabRequestModel).status == LabRequestStatus.pending)
          .length;
      final inProgressRequests = requests
          .where((r) => (r as LabRequestModel).status == LabRequestStatus.inProgress)
          .length;

      // أنواع الفحوصات الأكثر طلباً
      final testTypeCounts = <String, int>{};
      for (final req in requests) {
        final testType = (req as LabRequestModel).testType;
        testTypeCounts[testType] = (testTypeCounts[testType] ?? 0) + 1;
      }

      final topTestTypes = testTypeCounts.entries.toList()
      ...
      setState(() {
        _stats = {
          ...
          'topTestTypes': topTestTypes.take(5).toList(),
        };
      });
    } catch (e) {
      if (mounted) {
                ...
                child: Row(
                  children: [
                    CircleAvatar(
                      ...
                      child: const Icon(Icons.science, size: 40),
                    ),
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              Expanded(
                child: _buildStatCard(
                  'قيد التنفيذ',
                  _stats['inProgressRequests']?.toString() ?? '0',
          ...
          const SizedBox(height: 16),
          if (_stats['topTestTypes'] != null &&
              (_stats['topTestTypes'] as List).isNotEmpty)
            ...(_stats['topTestTypes'] as List).map((entry) {
              final testType = entry.key as String;
